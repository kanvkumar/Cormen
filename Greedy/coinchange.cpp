...
int main()
{
	...
	//cin.tie(NULL);
	//cout.tie(NULL);
	int n,j,i,coins[100001],k,opt[100001];
    //cin>>n>>k;//n is amount , k is denominations available
    fs(n);fs(k);
    for (i = 0; i<k; ++i)
    {
        fs(coins[i]);
    ...
    for(i=1;i<=n;i++)
    {
        opt[i]=i;//considering to be made with 1 value coins
        for(j=0;j<k;j++)
        {
            if((i>=coins[j]) && (opt[i-coins[j]]+1)<opt[i])
            {
